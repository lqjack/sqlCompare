...
public class QueryTree {
	...
	private void recDisplayTree(TreeNode thisNode, int level, int childNumber) {
		...
		System.out.println(" siteID=" + thisNode.getSiteID() + " nodeID="
				+ thisNode.getNodeID() + " classId=" + thisNode.getClassID());
	...
	private void genTreeListByNode(TreeNode node) {
		...
		n.parentNodeID = ((node.getParent() == null) ? -1 : node.getParent()
				.getNodeID());
	...
	public void genSelectTree(Select select) {
		...
		List<String> projectionItemsList = selectItemFinder
				.getSelectItemsList(select);
		String attributes =null;
		TreeNode curNode = null;
		for (int i = 0; i < projectionItemsList.size(); i++) {
			ProjectionNode node = new ProjectionNode();
			node.setNodeName("Projection");
			attributes += projectionItemsList.get(i)
					+ ((i < projectionItemsList.size()) ? "," : "");
		...
		for (int i = 0; i < joinList.size(); ++i) {
			...
			node2.addAttribute(joinList.get(i).leftColumn,
					joinList.get(i).rigthColumn);
	...
	private void localization(LeafNode leafNode) {
		...
		List<String> subTableList = (List<String>) gdd
				.getTableFragList(tableName);
