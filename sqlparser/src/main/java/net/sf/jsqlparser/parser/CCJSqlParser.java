...
public class CCJSqlParser implements CCJSqlParserConstants {
	...
	final public Init init() throws ParseException {
		...
		if (jj_2_1(2147483647)) {
			fileName = jj_consume_token(S_IDENTIFIER);
		} else if (jj_2_2(2147483647)) {
			fileName = jj_consume_token(S_PATH);
		} else if (jj_2_3(2147483647)) {
			fileName = jj_consume_token(S_PATH2);
		} else {
			jj_consume_token(-1);
			throw new ParseException();
		}
	...
	final public SetSite setSite() throws ParseException {
		...
		Token ip = null;
		jj_consume_token(K_SETSITE);
		siteName = relObjectName();
		siteNameList.add(siteName);
		jj_consume_token(K_ON);
		ip = jj_consume_token(S_IP);
		ipList.add(ip);
		jj_consume_token(92);
		port = jj_consume_token(S_INTEGER);
		portList.add(port);
		label_1: while (true) {
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
			case 93:
	...
	final public VerticalFragment verticalFragment() throws ParseException {
		...
		List columns = null;
		List all_columns = new ArrayList();
		jj_consume_token(K_VERTICAL);
		jj_consume_token(K_FRAGMENT);
		jj_consume_token(K_ON);
		table = table();
		...
		fragment.setColumns(all_columns);
		{
			if (true)
				return fragment;
		}
